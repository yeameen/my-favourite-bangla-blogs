class AssetsBundle         
  ...
  def initialize(names, ext, options = {})
    ext = extract_extension!(names) if ext.nil? || ext.empty?
    @names, @ext = names, ext.sub(/^\./, '')
    self.options = options
  end
  
  def extract_extension!(names)
    returning File.extname(names) do |ext|
      names.replace File.basename(names, ext)
    end
  end
  ...
  def basedir(type = nil)
    File.expand_path(RAILS_ROOT + path(type || @ext))
  end  

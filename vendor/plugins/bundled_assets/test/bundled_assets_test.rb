...
dirname = "/" + File.dirname(__FILE__).sub(RAILS_ROOT, '')
AssetsBundle.options.merge!(:paths => { :css => dirname, :js => dirname })
...
class BundledAssetsFilenamesTest < Test::Unit::TestCase 
  def setup 
    @bundle = AssetsBundle.new 'a,b,moo.fx', 'js'
  end 
  
  def test_should_find_filenames_with_dots
    assert_equal 2, @bundle.filenames.size
  end
end
